...
pub fn run(inputs: &[(String, PathBuf)], out: &PathBuf) {
    ...
    for (os, path) in inputs {
        ...
        let file = syn::parse_file(&src)
            .unwrap_or_else(|e| panic!("xtask merge: cannot parse {}: {e}", path.display()));
    ...
    let parsed = syn::parse_file(&full)
        .unwrap_or_else(|e| panic!("xtask merge: assembled output failed to parse: {e}"));
