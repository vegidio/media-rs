...
//! codec/support libraries they depend on) for the current target and links them
//! statically into the crate.
//!
//! The raw FFI bindings are **not** generated here — they are pre-generated by the
//! `xtask` crate across a per-OS CI matrix and committed to `src/sys/bindings.rs`, so
//! building this crate needs neither `bindgen` nor `libclang`.
//!
...
//! afterward. On GNU linkers the whole set is wrapped in `--start-group`/`--end-group`
